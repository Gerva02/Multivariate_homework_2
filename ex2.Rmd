...
```
```{r}
t(table(glass$type)) %>% apply(1,function(x) x/sum(x))
```
...
We can check how homogeneous the groups are by seeing if our linear predictor predicts equally well all of the classes. We do that by printing the confusion matrix with bouth the absolute:
```{r}
lda.pred  <-predict(lda.fit,glass)
...
From these table it is clear that the most heterogeneous class are the Head,WinF, WinF which are not so coincedly the most common kinds of glass, this means .
Meanwhile the other 3 are the least homogenus with Veh being the least since every observation belonging to that group has been classified somewhere else.
Since we don't have any more data is difficult to understand wether more observation could imporve significantly the result or their eterogenity is due to basic carateristics of the materials.
...
points(means.hat.z[,1],means.hat.z[,2],pch=21,bg=lookup,cex=1.5 )
```
